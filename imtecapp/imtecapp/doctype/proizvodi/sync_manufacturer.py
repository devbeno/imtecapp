...
def update_prestashop_manufacturer_name(manufacturer_id, new_name):
    """Update the manufacturer name in PrestaShop for multiple languages and update related Frappe records."""
    try:
        # Retrieve PrestaShop settings and initialize PrestaShop WebService
        settings = get_prestashop_settings()
        prestashop = PrestaShopWebServiceDict(settings["presta_url"], settings["presta_key"])

        # Fetch the existing manufacturer data from PrestaShop
        manufacturer_data = prestashop.get(f"manufacturers/{manufacturer_id}")

        # Check if the name supports multiple languages (lang 1 and lang 2)
        if 'name' in manufacturer_data['manufacturer']:
            if isinstance(manufacturer_data['manufacturer']['name'], dict) and 'language' in manufacturer_data['manufacturer']['name']:
                for lang in manufacturer_data['manufacturer']['name']['language']:
                    if lang['attrs']['id'] == '1':
                        lang['value'] = new_name  # Update for language 1
                    elif lang['attrs']['id'] == '2':
                        lang['value'] = new_name  # Update for language 2
            else:
                # Fallback in case there's no language-specific structure, update single language
                manufacturer_data['manufacturer']['name'] = new_name

        # Remove the 'link_rewrite' field as it's not writable for manufacturers
        if 'link_rewrite' in manufacturer_data['manufacturer']:
            del manufacturer_data['manufacturer']['link_rewrite']

        # Remove other non-writable fields if necessary
        non_writable_fields = ['date_add', 'date_upd']
        for field in non_writable_fields:
            if field in manufacturer_data['manufacturer']:
                del manufacturer_data['manufacturer'][field]

        # Log the payload being sent to PrestaShop for debugging
        print(f"Updating manufacturer ID {manufacturer_id} with new name '{new_name}' in multiple languages")
        print(f"Payload being sent: {manufacturer_data}")

        # Send the update request to PrestaShop
        response = prestashop.edit(f"manufacturers/{manufacturer_id}", manufacturer_data)

        # Check if the response is successful
        if 'manufacturer' in response:
            print(f"Manufacturer {manufacturer_id} successfully updated in PrestaShop.")

            # Update all Proizvodi records with this manufacturer name in Frappe
            frappe.db.sql(
                """
                UPDATE `tabProizvodi`
                SET prestashop_manufacturer_id = %s
                WHERE proizvodjac = %s
                """,
                (manufacturer_id, new_name),
            )
            frappe.db.commit()

            print(f"Updated Proizvodi records with manufacturer name '{new_name}' to have PrestaShop manufacturer ID {manufacturer_id}")
            return True
        else:
            print(f"Failed to update manufacturer {manufacturer_id} in PrestaShop. Response: {response}")
            return False

    except Exception as e:
        frappe.logger().error(f"Error updating manufacturer name in PrestaShop: {str(e)}")
        print(f"Error updating manufacturer name in PrestaShop: {str(e)}")
        return False
...
def find_prestashop_manufacturer_by_name(name):
    ...
    try:
        ...
        manufacturers = prestashop.get("manufacturers", options={"filter[name]": name, "display": "full"})
...
def create_prestashop_manufacturer(manufacturer):
    ...
    try:
        ...
        prestashop_manufacturer_id = response["manufacturer"]["id"]
