...
from imtecapp.imtecapp.doctype.proizvodi.sync_categorie import (
    ...
    find_prestashop_category_name_by_id,
...
from imtecapp.imtecapp.doctype.proizvodi.sync_manufacturer import (
    ...
    find_prestashop_manufacturer_name_by_id,
...
def find_prestashop_product_by_reference(settings, reference, sync_log=None):
    ...
    add_message_to_log(sync_log, f"Response Text: {truncate_message(response.text, max_length=200)}")
    ...
    if response.status_code == 200:
        try:
            ...
            for element in [
                ...
                # Add check for 'language[@id='1']' and 'language[@id='2']' keys to avoid Undefined array key warning
...
def sync_product_to_prestashop(art_sifra):
    ...
    sync_log = create_sync_log_record("Sync Product")  # Create sync log
    ...
    try:
        ...
        new_product_data = {
            ...
            "stanje": product.stanje,  # Stock quantity field
            "kataloski": product.kataloski,
            "prestashop_id": product.prestashop_id,
            "grupanaziv": product.grupanaziv,
            "status": product.status,
        }

        # Step 1: Sync the category
        group_name = product.grupanaziv
        prestashop_category_id = product.prestashop_category_id
        ...
        if prestashop_category_id:
            ...
            if existing_category_name and existing_category_name != group_name:
                ...
                if updated:
                    log_details += f"Updated PrestaShop category '{prestashop_category_id}' with new name '{group_name}'.\n"
                else:
                    log_details += f"Failed to update category name for '{group_name}' in PrestaShop.\n"
            else:
                log_details += f"Category '{group_name}' already synced with PrestaShop ID {str(prestashop_category_id)}.\n"
        else:
            ...
            if not prestashop_category_id:
                ...
                prestashop_category_id = create_prestashop_category(group_name, sync_log)
                if prestashop_category_id:
                    log_details += f"Created new category '{group_name}' in PrestaShop with ID: {prestashop_category_id}.\n"
                else:
                    log_details += f"Failed to create new category '{group_name}' in PrestaShop.\n"
            else:
                log_details += f"Matched Frappe category '{group_name}' with PrestaShop Category ID: {prestashop_category_id}.\n"

        # Make sure prestashop_category_id is not None
        if prestashop_category_id is None:
            log_details += f"Error: PrestaShop Category ID for '{group_name}' is None. Sync failed.\n"
            raise ValueError("PrestaShop Category ID cannot be None.")

        # Assign the found or created category ID to the product
        new_product_data["prestashop_category_id"] = prestashop_category_id

        # Step 2: Sync the manufacturer
        manufacturer_name = product.proizvodjac
        prestashop_manufacturer_id = product.prestashop_manufacturer_id
        ...
        if prestashop_manufacturer_id:
            ...
            if existing_manufacturer_name and existing_manufacturer_name != manufacturer_name:
                ...
                if updated:
                    log_details += f"Updated PrestaShop manufacturer '{prestashop_manufacturer_id}' with new name '{manufacturer_name}'.\n"
                else:
                    log_details += f"Failed to update manufacturer name for '{manufacturer_name}' in PrestaShop.\n"
            else:
                log_details += f"Manufacturer '{manufacturer_name}' already synced with PrestaShop ID {str(prestashop_manufacturer_id)}.\n"
        else:
            ...
            if not prestashop_manufacturer_id:
                # Create a new manufacturer if not found
                prestashop_manufacturer_id = create_prestashop_manufacturer({"proizvodjac": manufacturer_name})
                if prestashop_manufacturer_id:
                    log_details += f"Created new manufacturer '{manufacturer_name}' in PrestaShop with ID: {prestashop_manufacturer_id}.\n"
                else:
                    log_details += f"Failed to create new manufacturer '{manufacturer_name}' in PrestaShop.\n"
            else:
                log_details += f"Matched Frappe manufacturer '{manufacturer_name}' with PrestaShop Manufacturer ID: {prestashop_manufacturer_id}.\n"

        # Ensure prestashop_manufacturer_id is not None
        if prestashop_manufacturer_id is None:
            log_details += f"Error: PrestaShop Manufacturer ID for '{manufacturer_name}' is None. Sync failed.\n"
            raise ValueError("PrestaShop Manufacturer ID cannot be None.")

        # Assign the found or created manufacturer ID to the product
        new_product_data["prestashop_manufacturer_id"] = prestashop_manufacturer_id

        # Step 3: Sync the rest of the product data (like before)
        ...
        if prestashop_id:
            existing_product_data = fetch_existing_product_info(settings, prestashop_id, sync_log)
            if existing_product_data:
                ...
                # Generate XML payload and update product in PrestaShop
                ...
                if response.status_code in [200, 201]:
                    ...
                    # Update stock after successful product sync
                    stock_available_id = retrieve_stock_available_id(settings, prestashop_id, sync_log)
                    if stock_available_id:
                        update_product_stock_quantity(
                            settings, stock_available_id, prestashop_id, new_product_data["stanje"], sync_log
                        )
                    ...
                    log_details += f"Response: {truncate_message(response.text, max_length=200)}\n"
            ...
            if prestashop_id:
                ...
                stock_available_id = retrieve_stock_available_id(settings, prestashop_id, sync_log)
                if stock_available_id:
                    update_product_stock_quantity(settings, stock_available_id, prestashop_id, new_product_data["stanje"], sync_log)
                update_sync_log_record(sync_log, log_details, status="Success")
            else:
                log_details += f"Failed to insert product {new_product_data['art_sifra']} into PrestaShop.\n"
                update_sync_log_record(sync_log, log_details, status="Failed")
...
# def sync_category_to_prestashop(group_name, sync_log):
#     """Sync category to PrestaShop and return the new category ID."""
#     try:
...
#         # Find the category in PrestaShop
#         prestashop_category_id = find_prestashop_category_by_name(group_name)
#         if not prestashop_category_id:
#             # Create the category in PrestaShop if not found
#             prestashop_category_id = create_prestashop_category({"grupanaziv": group_name})

#         if prestashop_category_id:
#             # Update the local database with the new PrestaShop ID
#             frappe.db.sql(
#                 """
#                 UPDATE `tabProizvodi`
...
#     except Exception as e:
#         add_message_to_log(sync_log, f"Failed to sync category {group_name}: {e}")
#         raise e
            

# def sync_manufacturer_to_prestashop(manufacturer_name, sync_log):
#     """Sync manufacturer to PrestaShop and return the new manufacturer ID."""
#     try:
...
#         # Find the manufacturer in PrestaShop
#         prestashop_manufacturer_id = find_prestashop_manufacturer_by_name(manufacturer_name)
#         if not prestashop_manufacturer_id:
#             # Create the manufacturer in PrestaShop if not found
#             prestashop_manufacturer_id = create_prestashop_manufacturer({"proizvodjac": manufacturer_name})
...
#             )
#         else:
#             add_message_to_log(sync_log, f"Failed to process manufacturer '{manufacturer_name}'")

#         return prestashop_manufacturer_id
#     except Exception as e:
#         add_message_to_log(sync_log, f"Failed to sync manufacturer {manufacturer_name}: {e}")
#         raise e
...
#     try:
#         # Update the sync log to "In Progress"
#         update_sync_log_record(sync_log, log_details, status="In Progress")
...
#                 update_sync_log_record(sync_log, log_details, status="Failed")  # Mark as failed
...
#                 update_sync_log_record(sync_log, log_details, status="Failed")  # Set status to failed
...
def insert_product_to_prestashop(art_sifra):
    ...
    try:
        ...
        # Ensure prestashop_category_id is not None
        if prestashop_category_id is None:
            log_details += f"Error: PrestaShop Category ID for '{group_name}' is None. Insert failed.\n"
            raise ValueError("PrestaShop Category ID cannot be None.")
        ...
        # Ensure prestashop_manufacturer_id is not None
        if prestashop_manufacturer_id is None:
            log_details += f"Error: PrestaShop Manufacturer ID for '{manufacturer_name}' is None. Insert failed.\n"
            raise ValueError("PrestaShop Manufacturer ID cannot be None.")

        # Assign the found or created manufacturer ID to the product
        new_product_data["prestashop_manufacturer_id"] = prestashop_manufacturer_id

        ### Step 3: Insert product into PrestaShop ###
...
def rename_prestashop_product_by_reference(art_sifra):
    ...
    try:
        ...
        # Ensure prestashop_category_id is not None
        if prestashop_category_id is None:
            log_details += f"Error: PrestaShop Category ID for '{group_name}' is None. Rename failed.\n"
            raise ValueError("PrestaShop Category ID cannot be None.")
        ...
        # Ensure prestashop_manufacturer_id is not None
        if prestashop_manufacturer_id is None:
            log_details += f"Error: PrestaShop Manufacturer ID for '{manufacturer_name}' is None. Rename failed.\n"
            raise ValueError("PrestaShop Manufacturer ID cannot be None.")

        # Assign the found or created manufacturer ID to the product
        new_product_data["prestashop_manufacturer_id"] = prestashop_manufacturer_id

        ### Step 3: Rename the product in PrestaShop ###
        settings = retrieve_prestashop_settings()
        prestashop_id = find_prestashop_product_by_reference(settings, new_product_data["art_sifra"], sync_log=sync_log)
        ...
        if prestashop_id:
            ...
            if response.status_code in [200, 201]:
                ...
                update_sync_log_record(sync_log, log_details, status="Success")  # Mark as Success
            else:
